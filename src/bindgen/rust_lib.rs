...
struct FullModule {
    ...
    pub content: Vec<syn::Item>,
}

impl syn::parse::Parse for FullModule {
...
pub fn parse_src<F>(src_file: &Path, items_callback: &mut F) -> ParseResult
where
    F: FnMut(&str, &Vec<syn::Item>),
{
    let src_parsed: FullModule = {
        let mut s = String::new();
        let mut f = File::open(src_file)
            .map_err(|_| format!("parsing: cannot open file `{:?}`", src_file))?;
        f.read_to_string(&mut s)
            .map_err(|_| format!("parsing: cannot open file `{:?}`", src_file))?;
        syn::parse_str(&s).map_err(|msg| format!("parsing:\n{}", msg))?
    };

    items_callback("", &src_parsed.content);
    ...
    F: FnMut(&str, &Vec<syn::Item>),
{
    ...
    match crate_src {
        Some(crate_src) => parse_mod(pkg, crate_src.as_path(), context),
        None => {
    ...
    F: FnMut(&str, &Vec<syn::Item>),
{
    let mod_parsed = {
        if !context.cache_expanded_crate.contains_key(&pkg.name) {
            ...
            let i: FullModule = syn::parse_str(&s)
                .map_err(|msg| format!("parsing crate `{}`:\n{}", pkg.name, msg))?;
            context
                .cache_expanded_crate
                .insert(pkg.name.clone(), i.content);
        }
    ...
    F: FnMut(&str, &Vec<syn::Item>),
{
    ...
    for item in items {
        match item {
            syn::Item::Mod(ref mod_def) => {
                if let Some((_, ref inline_items)) = mod_def.content {
            ...
            syn::Item::ExternCrate(extern_crate) => {
                let dep_pkg_name = extern_crate.ident.to_string();
...
fn parse_mod<F>(pkg: &PackageRef, mod_path: &Path, context: &mut ParseLibContext<F>) -> ParseResult
where
    F: FnMut(&str, &Vec<syn::Item>),
{
    let mod_parsed = {
        ...
        if !context.cache_src.contains_key(&owned_mod_path) {
            ...
            let i: FullModule = syn::parse_str(&s)
                .map_err(|msg| format!("parsing crate `{}`:\n{}", pkg.name, msg))?;
            context.cache_src.insert(owned_mod_path.clone(), i.content);
        }
    ...
    process_mod(pkg, mod_dir, &mod_parsed, context)
}
    ...
    F: FnMut(&str, &Vec<syn::Item>),
{
    ...
    for item in items {
        match item {
            syn::Item::Mod(ref inline_items) => {
                let next_mod_name = inline_items.ident.to_string();

                if let Some((_, ref inline_items)) = inline_items.content {
                    process_mod(pkg, &mod_dir.join(&next_mod_name), inline_items, context)?;
                    continue;
                }
                ...
                if next_mod_path1.exists() {
                    parse_mod(pkg, next_mod_path1.as_path(), context)?;
                } else if next_mod_path2.exists() {
                    parse_mod(pkg, next_mod_path2.as_path(), context)?;
                } else {
                    // This should be an error, but is common enough to just elicit a warning
                    warn!(
